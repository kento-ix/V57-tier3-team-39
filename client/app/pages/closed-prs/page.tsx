...
import { LoadingOverlay, Modal, Text, Box } from "@mantine/core";
...
export default function ClosedPRsPage() {
  ...
  const fetchPRs = async () => {
    ...
    try {
      ...
      else setError("🚨 Unable to fetch pull requests.");
      setPrs([]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="flex-grow">
      <h1 className="p-3 text-4xl font-bold text-center mt-6" style={{ fontFamily: "var(--font-syne)" }}>
        ...
        <Box pos="relative">
          ...
          {error && (
            <div className="flex flex-col justify-center items-center h-48 gap-4 text-center">
              ...
              <button onClick={fetchPRs} className="px-4 py-2 bg-purple-500 text-white rounded-md hover:bg-purple-700">
                Retry
              </button>
            </div>
          )}
          ...
          {filteredPRs.length === 0 && prs.length > 0 && !error && (
            <div className="text-gray-600 text-2xl text-center p-8" role="status" aria-live="polite">
